...
/*9. Mostrar la relación de productos (Nombre) 
  ...
  */
  ...
  from vw_prod_cat vw1 join vw_prod_cat vw2 on vw1.CategoryID=vw2.CategoryID
  where vw1.QVendida=vw2

  select vw.CategoryID, vw.ProductID,vw.ProductName, vw.QVendida,MAX(vw.QVendida)
  from vw_prod_cat vw
  group by vw.CategoryID, vw.ProductID,vw.ProductName,vw.QVendida
...
Mostrar el país ,el orderid y  monto del pedido,  ordenado por monto de mayor a menor.*/
-- eliminar 
drop view  vw_monto_pais
...
--falta el order id 
select vw1.ShipCountry,max(vw1.monto)'mtotal'
from vw_monto_pais vw1
group by vw1.ShipCountry, vw1.OrderID 
...
group by vw.ShipCountry
 ...
 /*Mostrar la lista de proveedores (Suppliers) cuyos productos son 
  ...
  select s.SupplierID, s.CompanyName, sum (od.Quantity)'qvendida', o.OrderDate
  from Products p join Suppliers s on s.SupplierID=p.SupplierID 
  join [Order Details] od on od.ProductID= p.ProductID 
  join Orders o on o.OrderID=od.OrderID 
  where o.OrderDate = '2017'
    group by  s.SupplierID, s.CompanyName, o.OrderDate



	create view vw_venta_supplier as
  select  s.CompanyName, s.SupplierID,sum(od.Quantity)Qvendida
  from Suppliers s join Products p on s.SupplierID=p.SupplierID 
  join [Order Details] od on  p.ProductID = od.ProductID
  join Orders o on o.OrderID=od.OrderID 
  where year(o.OrderDate)=2017 --no comillas
 ...
 select c.CompanyName, o.OrderDate, c.Country,count(o.OrderID)'cant ordenes', sum(od.Quantity*od.UnitPrice*(1-od.Discount))'monto_acum' 
 from [Order Details] od join Orders o on od.OrderID= o.OrderID 
 join Customers c on o.CustomerID=c.CustomerID
 where year(o.OrderDate)=2017
 group by c.CompanyName, o.OrderDate, c.Country
 having count(o.OrderID) > avg(o.OrderID)  
